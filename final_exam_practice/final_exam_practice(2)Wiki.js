...
var parentElement = document.getElementById("wikiTableBody");

// Make URL for API call

var searchTerm = "Digital Humanities";
// var searchTerm = document.getElementById('searchTerm').value;
var url = `https://en.wikipedia.org/w/api.php?action=query&format=json&list=search&srsearch=${encodeURIComponent(searchTerm)}&origin=*`;
console.log(url);
...
function XMLrequest(link){ 
    ...
    connect.onreadystatechange = function(){ // check the state of the object I just created (connect)
        if(this.readyState == 4 && this.status == 200){ // conditional check
            ...
            listTexts(JSON.parse(this.responseXML));
        };
    };
    // open API call
